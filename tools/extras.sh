...
#Preamble: Some repos outside of cxr may have commits we need that take too
...
#
#  Any patches stored in the vendor/cxr/extras/patches folder must have a .patch extension
...
# colors
. $BASEDIR/vendor/cxr/tools/colors
...
function patch_it {
  ...
  if [ "$1" = "true" ]; then
   ...
   THISCOMMIT=$(cat $BASEDIR/vendor/cxr/extras/patches/${PATCH}.patch | sed -n '4,4p')

   if [ "$LASTCOMMIT" != "$THISCOMMIT" ] ; then  #Patch if not already applied
    git am $BASEDIR/vendor/cxr/extras/patches/${PATCH}.patch
   else
    echo -e "skipped $(echo $THISCOMMIT | sed -r 's/^.{9}//')"
   fi
...
case $device in
    ...
    s2ve | s2vep)
        # av patch
        PATCH="https://raw.githubusercontent.com/luk1337/build_tools/cxr/0001-FRAMEWORKS-AV-PATCH-for-s2ve-s2vep-device.patch"
        FOLDER=frameworks/av
        patch_it true
        # native patch
        PATCH="https://raw.githubusercontent.com/luk1337/build_tools/cxr/0001-FRAMEWORKS-NATIVE-PATCH-for-s2ve-s2vep-device.patch"
        FOLDER=frameworks/native
        patch_it true
        # bluetooth patch
        PATCH="https://raw.githubusercontent.com/luk1337/build_tools/cxr/0001-Add-BT-support-for-s2ve-s2vep.patch"
        FOLDER=hardware/broadcom/libbt
        patch_it true
        # hwc patch
        PATCH="https://raw.githubusercontent.com/luk1337/build_tools/cxr/0001-Fix-HWC-for-s2ve-s2vep.patch"
        FOLDER=external/chromium_org
        patch_it true
    ;;
    p705)
        #libstagefright: Add support for custom LPA buffer size in legacy LPAPlayer
        #cherries+=(1343_cxr) : this was lost during a gerrit rebuild, suggest the maintainer make a patch file for it
    ...
    tenderloin)
        ...
        FOLDER=cxrkages/apps/Bluetooth
