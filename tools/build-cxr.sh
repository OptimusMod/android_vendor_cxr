...
# PAC version
export PAC_VERSION_MAJOR="CXR"
export PAC_VERSION_MINOR="KK"
export PAC_VERSION_MAINTENANCE="Unofficial"
# Acceptable maintenance versions are; Stable, Official, Nightly or Unofficial

# PAC version logic
if [ -s ~/CXRname ]; then
    export CXR_MAINTENANCE=$(cat ~/CXRname)
else
    export CXR_MAINTENANCE="$CXR_VERSION_MAINTENANCE"
fi
export CXR_VERSION="$CXR_VERSION_MAJOR $CXR_VERSION_MINOR $CXR_MAINTENANCE"

usage()
{
    echo -e ""
    echo -e ${txtbld}"Usage:"${txtrst}
    echo -e "  build-cxr.sh [options] device"
    echo -e ""
    ...
    echo -e "    ./build-cxr.sh -c1 cooperve"
    echo -e ""
    exit 1
}

# Colors
. ./vendor/cxr/tools/colors

if [ ! -d ".repo" ]; then
    echo -e ${red}"No .repo directory found.  Is this an Android build tree?"${txtrst}
    exit 1
fi
if [ ! -d "vendor/cxr" ]; then
    echo -e ${red}"No vendor/pac directory found.  Is this a CXR build tree?"${txtrst}
    exit 1
fi
...
echo -e ${cya}"Building ${bldgrn}C ${bldppl}X ${bldblu}R ${bldylw}$CXR_VERSION"${txtrst}
...
# CXR device dependencies
echo -e ""
echo -e ${bldblu}"Looking for CXR product dependencies${txtrst}"${cya}
if [ "$opt_kr" -ne 0 ]; then
    vendor/cxr/tools/getdependencies.py "$device" "$opt_kr"
else
    vendor/cxr/tools/getdependencies.py "$device"
fi
echo -e "${txtrst}"
...
if [ "$opt_clean" -eq 1 ]; then
    ...
    . build/envsetup.sh && lunch "cxr_$device-userdebug";
...
if [ "$opt_lrd" -ne 0 ]; then
    ...
    export CXR_LOW_RAM_DEVICE=true
    echo -e ""
else
    unset CXR_LOW_RAM_DEVICE
fi
...
if [ "$opt_fetch" -ne 0 ]; then
    ./vendor/cxr/tools/extras.sh $device
fi
...
rm -f $OUTDIR/target/product/$device/cxr_*-ota*.zip
